...
function processRows(rawRows) {
  return rawRows.map(r => ({
    BirthWeight: r['LowBirthWeightIndLowBirthWeightIndDesc'] === 'Low birth weight (<2500g)' ? '< 5.5 lbs' : '5.5 lbs+',
...
function CountyChart({ title, data, height = 300 }) {
  return (
    <div>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#374151', textAlign: 'center', marginBottom: 4 }}>{title}</div>
      {data && data.length > 0 ? (
        <ResponsiveContainer width="100%" height={height}>
          <BarChart data={data} margin={{ top: 4, right: 8, bottom: 52, left: 40 }}>
            ...
            <Tooltip formatter={(val, name) => [val.toLocaleString(), name]} labelFormatter={l => `Age: ${l}`} />
...
export default function OhioBirthData() {
    ...
    Promise.all([
      ...
      fetch('https://cdn.jsdelivr.net/npm/us-atlas@3/counties-10m.json')
        ...
        .then(topo => {
          ...
          return { type: 'FeatureCollection', features: all.features.filter(f => String(f.id).startsWith('39')) }
        }),
    ])
      .then(([data, geo]) => { setBirthData(data); setGeoJson(geo); setLoading(false) })
      .catch(err => { setError(err.message); setLoading(false) })
  }, [])

  const mapStats = useMemo(() => getMapStats(birthData), [birthData])
  const colorFn = useMemo(() => makeColorFn(mapStats), [mapStats])
  const exploreData = useMemo(() => getChartData(birthData, selectedCounty, ['2018'])['2018'] || [], [birthData, selectedCounty])
  const trendData = useMemo(() => getChartData(birthData, selectedCounty, TREND_YEARS), [birthData, selectedCounty])

  const geoJsonStyle = useCallback((feat) => ({
    fillColor: colorFn(mapStats[feat.properties.name] ?? 0),
    fillOpacity: 0.65, color: '#444', weight: 1,
  }), [mapStats, colorFn])
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ background: '#1e3a5f', color: 'white', padding: '10px 24px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Interactive Ohio Birth Data</h1>
          <p style={{ margin: '1px 0 0', fontSize: 11, opacity: 0.7 }}>Click a county to explore birth weight by maternal age</p>
        </div>
        <Link href="/" style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', textDecoration: 'none' }}>← All visualizations</Link>
      </div>

      <div style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '0 20px', display: 'flex', flexShrink: 0 }}>
        {[
          ['explore',    'Explore Data (2018)',      <BarChart2 key="i1" size={14} />],
          ['trend',      '5 Year Trend (2014–2018)', <TrendingUp key="i2" size={14} />],
          ['background', 'Background',               <Info key="i3" size={14} />],
        ].map(([id, label, icon]) => (
      ...
      {activeTab === 'background' && (
        <div style={{ flex: 1, overflowY: 'auto', background: '#f9fafb', display: 'flex', justifyContent: 'center', padding: '48px 24px' }}>
          <div style={{ maxWidth: 720, width: '100%', lineHeight: 1.7 }}>
            <h2 style={{ fontSize: 26, fontWeight: 800, margin: '0 0 8px', color: '#111827' }}>About This Project</h2>
            ...
            <Section title="Summary">
              <p>This dashboard visualizes Ohio live birth data across all 88 counties, allowing users to explore the relationship between maternal age groups and birth weight outcomes. Counties are shaded by percentage of low birth weight births (&lt;2,500g / ~5.5 lbs), with darker red indicating higher rates.</p>
              <p style={{ marginTop: 12 }}>Originally built in R with Shiny and Leaflet, this version has been rebuilt as a static React application — no server required.</p>
            </Section>
            <Section title="Data Source">
              <p>Data was obtained from the <a href="http://publicapps.odh.ohio.gov/EDW/DataBrowser/Browse/OhioLiveBirths" style={{ color: '#2563eb' }} target="_blank" rel="noreferrer">Ohio Department of Health Public Data Warehouse</a>. The dataset covers Ohio live births from 2006–2018.</p>
            </Section>
            <Section title="Key Terms">
              <dl style={{ margin: 0 }}>
                {[
                  ...
                  ['Quantile Color Scale', 'Counties are colored in six quantile-based steps so relative differences are visible regardless of absolute range.'],
                ].map(([term, def]) => (
            ...
            <div style={{ marginTop: 40, padding: '16px 20px', background: '#fffbeb', borderLeft: '4px solid #f59e0b', borderRadius: 4, fontSize: 13, color: '#78350f' }}>
              <strong>Disclaimer:</strong> These data were provided by the Ohio Department of Health. The Department specifically disclaims responsibility for any analyses, interpretations, or conclusions drawn from this data.
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab !== 'background' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          <div style={{ position: 'relative', overflow: 'hidden' }}>
            <MapContainer center={[39.9, -82.1]} zoom={7} zoomControl={false} dragging={false}
              scrollWheelZoom={false} doubleClickZoom={false}
              style={{ height: '100%', width: '100%' }} minZoom={7} maxZoom={7}>
              <GeoJSON key="ohio-counties" data={geoJson} style={geoJsonStyle} onEachFeature={onEachFeature} />
            </MapContainer>
            <div style={{ position: 'absolute', bottom: 16, right: 16, background: 'rgba(255,255,255,0.95)', padding: '10px 12px', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', zIndex: 1000, fontSize: 11 }}>
