    ...
    <div style={{ minHeight: '100vh', background: '#d7edf9', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <Slider label="Cheat Die P(6)" min={0.1} max={1.0} step={0.01} value={cheatEffect} onChange={setCheatEffect} />
          <Slider label="Number of Cheat Dice" min={1} max={50} step={1} value={numCheat}
            onChange={v => { setNumCheat(v); if (v > totalDice) setTotalDice(v) }} />
          <Slider label="Total Dice Population" min={1} max={500} step={1} value={safeTotalDice}
            onChange={v => setTotalDice(Math.max(v, numCheat))} />
          <Slider label="Consecutive Sixes Rolled" min={1} max={10} step={1} value={selectedRolls} onChange={setSelectedRolls} />

          <div style={{ marginTop: '20px', padding: '16px', background: '#eff6ff', borderRadius: '8px', borderLeft: '4px solid #2563eb' }}>
        ...
        <div>
          ...
          <p style={{ margin: '0 0 24px', fontSize: '13px', color: '#6b7280', lineHeight: 1.6 }}>
            ...
            six{selectedRolls > 1 ? 'es' : ''}. The red dot shows the probability they picked the cheat die.
          </p>
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={data} margin={{ top: 10, right: 24, bottom: 36, left: 24 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis dataKey="rolls" tickCount={10} interval={0}>
                <Label value="Number of Consecutive Sixes Rolled" position="insideBottom" offset={-20} style={{ fontSize: 12, fill: '#6b7280' }} />
              </XAxis>
              <YAxis domain={[0, 1]} tickFormatter={v => `${(v * 100).toFixed(0)}%`}>
                <Label value="P(Cheat Die)" angle={-90} position="insideLeft" offset={-8} style={{ fontSize: 12, fill: '#6b7280' }} />
              </YAxis>
              <Tooltip
                formatter={v => [`${(v * 100).toFixed(2)}%`, 'P(Cheat Die)']}
                labelFormatter={l => `${l} consecutive six${l > 1 ? 'es' : ''}`}
              />
              <Line type="monotone" dataKey="prob" stroke="#2563eb" strokeWidth={2.5}
                dot={<CustomDot selectedRolls={selectedRolls} />} activeDot={false} isAnimationActive={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
